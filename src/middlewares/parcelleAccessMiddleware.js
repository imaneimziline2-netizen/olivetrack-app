import Parcelle from "../modules/parcelles/parcelle.model.js";
...
export const checkParcelleAccess = (paramName = "parcelleId") => {
    return async (req, res, next) => {
        try {
            ...
            if (!parcelleId) {
                return res.status(400).json({ message: "ID de parcelle manquant" });
            }
            ...
            if (!isOwner && !isAdmin) {
                return res.status(403).json({ 
                    message: "Accès refusé : cette parcelle ne vous appartient pas" 
                });
            }
