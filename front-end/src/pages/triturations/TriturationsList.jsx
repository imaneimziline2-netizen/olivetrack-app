...
function TriturationsList() {
    ...
    const handleDelete = async () => {
        ...
        if (deleteTrituration.fulfilled.match(result)) {
            ...
            setDeleteError(result.payload || "Impossible de supprimer cette trituration");
        }
    };

    const totalOlives = triturations.reduce((sum, t) => sum + (t.quantite || 0), 0);
    const totalHuile = triturations.reduce((sum, t) => sum + (t.quantitéHuile || 0), 0);
    ...
    return (
        <div className="p-6 space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-2xl font-bold text-gray-900">Opérations de Trituration 🫒</h1>
                    <p className="text-sm text-gray-500 mt-1">Transformation des olives en huile</p>
                </div>
                <button
                    onClick={() => navigate(activeParcelleId ? `/triturations/new?parcelleId=${activeParcelleId}` : "/triturations/new")}
            ...
            <div className="flex items-center justify-between bg-white border border-gray-200 rounded-xl p-4">
                ...
                <div className="flex items-center gap-4 text-xs font-semibold">
                    <span>Olives : <strong>{totalOlives.toLocaleString()} kg</strong></span>
                    <span>Huile : <strong>{totalHuile.toLocaleString()} L</strong></span>
                    <span className="bg-emerald-50 text-green-700 px-3 py-1 rounded-lg">Rendement moyen : {avgRendement}%</span>
                </div>
            </div>
            ...
            <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
                ...
                {loading ? (
                        ...
                        <tbody className="divide-y">
                            {triturations.map((t) => (
                                <tr key={t._id}>
                                    ...
                                    <td className="px-4 py-2.5">{t.quantitéHuile} L</td>
                                    <td className="px-4 py-2.5 font-bold text-green-700">{t.rendement}%</td>
                                    <td className="px-4 py-2.5 text-right">
                                        <button onClick={() => setDeleteId(t._id)} className="text-xs text-red-600">
                                            Supprimer
                                        </button>
                                    </td>
                                </tr>
                    ...
                    <p className="text-center text-gray-400 py-12 text-sm">Aucune trituration enregistrée.</p>
                )}
            </div>
            ...
            {deleteId && (
                <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
                    <div className="bg-white rounded-xl p-6 w-80">
                        <h3 className="font-semibold mb-2">Supprimer cette trituration ?</h3>
                        <p className="text-xs text-gray-500 mb-4">Le stock d'olives sera restitué.</p>
