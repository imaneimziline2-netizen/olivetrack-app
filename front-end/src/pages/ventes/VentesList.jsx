...
export default function VentesList() {
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    await dispatch(deleteVente(deleteId));
    setDeleteId(null);
    if (selectedParcelleId) dispatch(fetchVentes(selectedParcelleId));
  };
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Suivi des ventes d'huile d'olive</p>
        </div>
        <button
          onClick={() =>
            navigate(`/ventes/new${selectedParcelleId ? `?parcelleId=${selectedParcelleId}` : ''}`)
          }
      ...
      <div className="bg-white border border-gray-100 rounded-2xl shadow-xs p-4 mb-6">
        <label className="block text-sm font-medium text-gray-700 mb-2">Parcelle</label>
        <select
      ...
      {!selectedParcelleId ? (
        <div className="bg-white border border-gray-100 rounded-2xl shadow-xs p-12 text-center">
          ...
          <p className="text-gray-500 text-sm">Sélectionnez une parcelle pour voir ses ventes</p>
        </div>
      ) : loading ? (
        ...
        <>
          ...
          {ventes.length > 0 && (
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
              ...
              <div className="bg-white border border-gray-100 rounded-2xl shadow-xs p-4">
                <p className="text-xs text-gray-500 mb-1">Quantité vendue</p>
                <p className="text-2xl font-bold text-[#059669]">{totalQte.toFixed(1)} L</p>
              </div>
              <div className="bg-white border border-gray-100 rounded-2xl shadow-xs p-4">
                <p className="text-xs text-gray-500 mb-1">Revenu total</p>
                <p className="text-2xl font-bold text-[#059669]">{totalRevenu.toFixed(2)} MAD</p>
              </div>
            </div>
          )}
          ...
          <div className="bg-white border border-gray-100 rounded-2xl shadow-xs overflow-hidden">
            {ventes.length === 0 ? (
              <div className="p-12 text-center">
                ...
                <p className="text-gray-500 text-sm mb-4">Aucune vente enregistrée</p>
                <button
                  onClick={() => navigate(`/ventes/new?parcelleId=${selectedParcelleId}`)}
              ...
              <table className="w-full text-sm">
                <thead className="bg-gray-50 border-b border-gray-100">
                  <tr>
                    ...
                    <th className="text-right px-5 py-3 font-medium text-gray-600">Quantité (L)</th>
                    <th className="text-right px-5 py-3 font-medium text-gray-600">Revenu (MAD)</th>
                    <th className="text-right px-5 py-3 font-medium text-gray-600">Prix/L</th>
                    ...
                    return (
                      <tr key={v._id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-5 py-3 text-right font-medium text-gray-800">
                          {v.quantiteVendue?.toFixed(1) || '-'}
                        </td>
                        <td className="px-5 py-3 text-right font-medium text-[#059669]">
                          {v.revenu?.toFixed(2) || '-'}
                        </td>
                        <td className="px-5 py-3 text-right text-gray-500">{prixUnitaire}</td>
      ...
      {deleteId && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl shadow-xl p-6 w-80">
            <h3 className="font-semibold text-gray-800 mb-2">Supprimer cette vente ?</h3>
