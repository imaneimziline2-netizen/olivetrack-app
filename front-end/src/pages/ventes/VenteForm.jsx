...
export default function VenteForm() {
  ...
  const prixUnitaire =
    form.quantiteVendue && form.revenu
      ? (parseFloat(form.revenu) / parseFloat(form.quantiteVendue)).toFixed(2)
      : null;
    ...
    <div className="max-w-xl mx-auto">
      ...
      <div className="bg-white border border-gray-100 rounded-2xl shadow-xs p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          {/* Revenu */}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Revenu (MAD)</label>
            <input
              type="number"
              name="revenu"
              value={form.revenu}
          ...
          {prixUnitaire !== null && (
            <div className="flex items-center gap-3 p-3 bg-[#edf7ee] border border-[#059669]/30 rounded-lg">
              ...
              <div>
                <p className="text-xs text-gray-500">Prix unitaire estimé</p>
                <p className="text-lg font-bold text-[#059669]">{prixUnitaire} MAD/L</p>
              </div>
            </div>
          )}
